 ...
 */
static inline int calc_contact_expires(
		contact_t *c, int expires_hdr, int local_time_now)
{
	unsigned int r = 0;
	if(expires_hdr >= 0)
		r = expires_hdr;
 ...
 */
static inline int update_contacts(struct sip_msg *req, struct sip_msg *rpl,
		udomain_t *_d, unsigned char is_star, int expires_hdr, str *public_id,
		int public_id_cnt, str *service_route, int service_route_cnt,
		int requires_nat)
{
	...
	for(h = rpl->contact; h; h = h->next) {
		if(h->type == HDR_CONTACT_T && h->parsed) {
			for(c = ((contact_body_t *)h->parsed)->contacts; c; c = c->next) {
				...
				ci.reg_state =
						PCONTACT_REGISTERED | PCONTACT_REG_PENDING
						| PCONTACT_REG_PENDING_AAR; //we don't want to add contacts that did not come through us (pcscf)
				...
				if(trust_bottom_via) {
					...
					alias_s.len =
							puri.params.len - (alias_start - puri.params.s) - 6;
					...
					if(p != NULL) {
						...
						port_s = p + 1;
						p = _strnistr(port_s, "~",
								alias_s.len - ci.received_host.len);
						if(p != NULL) {
							LM_DBG("alias(port) [%.*s]\n", (int)(p - port_s),
									port_s);
							memset(portbuf, 0, 5);
					...
					ci.received_host.len = ip_addr2sbuf(
							&req->rcv.src_ip, srcip, sizeof(srcip));
					ci.received_host.s = srcip;
					ci.received_port = req->rcv.src_port;
					ci.received_proto = req->rcv.proto;
					ci.searchflag = SEARCH_RECEIVED;
					LM_DBG("received from request: host [%.*s], port [%d] , "
						   "proto [%d]\n",
							ci.received_host.len, ci.received_host.s,
							ci.received_port, ci.received_proto);
				}
				...
				if(ul.get_pcontact(_d, &ci, &pcontact, 0)
					...
					if((expires - local_time_now)
						...
						LM_DBG("Updating contact: <%.*s>, old expires: "
							   "%" TIME_T_FMT
...
int save_pending(struct sip_msg *_m, udomain_t *_d)
{
	...
	if((sec_verify_params = cscf_get_security_verify(_m)) == NULL) {
		LM_DBG("Will save pending contact without security-verify "
			   "parameters\n");
	} else {
		if(sec_params) {
			// for REGISTER request try to set spi pc and spi ps from security-verify header
			sec_params->data.ipsec->spi_ps =
					sec_verify_params->data.ipsec->spi_us;
			sec_params->data.ipsec->spi_pc =
					sec_verify_params->data.ipsec->spi_uc;

			// Get from verify header pcscf server and client ports
			sec_params->data.ipsec->port_ps =
					sec_verify_params->data.ipsec->port_us;
			sec_params->data.ipsec->port_pc =
					sec_verify_params->data.ipsec->port_uc;

			LM_DBG("Will save pending contact with security-verify parameters, "
				   "spc_ps %u, spi_pc %u, port_ps %u, port_pc %u\n",
					sec_params->data.ipsec->spi_ps,
					sec_params->data.ipsec->spi_pc,
					sec_params->data.ipsec->port_ps,
					sec_params->data.ipsec->port_pc);
...
int save(struct sip_msg *_m, udomain_t *_d, int _cflags)
{
	...
	if((parse_headers(_m, HDR_CONTACT_F, 0) == -1) || !_m->contact) {
		LM_ERR("cannot get the Contact header from the SIP message in saving "
			   "action in PCSCF\n");
		goto error;
	}
	...
	if(!update_contacts(req, _m, _d, cb->star, expires_hdr, public_ids,
			   num_public_ids, service_routes, num_service_routes, 0)) {
		...
		if(num_public_ids > 0) {
			...
			while(i < num_public_ids && found_presentity_uri == 0) {
				...
				if(strncasecmp(public_ids[i].s, "tel:", 4) == 0) {
					...
					if(pv_parse_format(&public_ids[i], &presentity_uri_pv)
							< 0) {
			...
			if(found_presentity_uri != 1) {
				LM_ERR("Could not find routable URI in p_assoiated_uri list - "
					   "failed to subscribe");
				goto error;
			}
		} else {
			//Now some how check if there is a pua record and what the presentity uri is from there - if nothing there
			LM_DBG("No p_associated_uri in 200 OK this must be a de-register - "
				   "we ignore this - will unsubscribe when the notify is "
				   "received");
			goto done;
		}
	...
	if(public_ids && public_ids->s)
		pkg_free(public_ids);
	if(service_routes && service_routes->s)
		pkg_free(service_routes);
	return 1;
